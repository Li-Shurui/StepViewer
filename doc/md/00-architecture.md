...
| stride/scanline | 文件名 `_stride[N]` / `_scanline[N]`（解析失败则报错）> 用户在工具栏改过的值 > 当前格式的紧凑排布。紧凑帧能整除文件大小且名字没写 padding 时，两个框只读。 |
| 样本打包 | 扩展名对应格式的惯例 > 上次会话 > 满量程 16 位 |
| 显示变换 | 上次会话 > 线性（原样显示） |

格式一栏里扩展名压过会话状态，是必须的：`.Y8` 文件套用上次会话的 I420
会去读根本不存在的色度平面。文件名里的 `4096x3072` 只提供宽高；stride
和 scanline 由当前格式填成紧凑值，显示在工具栏里。若这一紧凑帧的大小
能整除文件，两个框只读——没有 padding 可填。名字里写了 `_stride[N]`，
或文件比紧凑帧大，框才可编辑。换格式会按新格式重新自动填，不会把
NV12 的 4096 带到 Bayer 上。`_stride[` / `_scanline[` 写了却不是合法
数字时，打开文件就报错。
...
| `PackedYuv444Decoder` | `yuvdecoders.cpp` | YUV444, AYUV |
...
（自动电平、灰世界、Gamma）、stride 与 scanline。字段是逐版本追加的，
读取时用 `atEnd()` 判断，旧状态文件仍可读。格式存的是 `id()` 字符串而非
下拉框序号，所以调整格式顺序不会让旧状态错位。显示预设存的是三个字段
而不是下拉框序号，同样不怕以后增删预设。紧凑文件的 stride 不从会话恢复，
以免把上次 NV12 的 4096 写进 Bayer 的只读框。
